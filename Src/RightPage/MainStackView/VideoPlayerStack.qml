...
Item {
    ...
    property bool isPageSwitching: false  // 跟踪页面切换状态
    ...
    StackView.onActivated: {
        // 切回到该页面时，确保重新挂载
        root_parent.isPageSwitching = true

        // 延迟执行，确保布局已完成
        Qt.callLater(function() {
            var videoRenderItem = root_parent.children[0].videoRenderItem
            if (videoRenderItem) {
            ...
            Qt.callLater(function() {
                // 页面切换完成后，恢复菜单状态
                root_parent.isPageSwitching = false
            });
        });
    }
    ...
    Item {
        ...
        onWidthChanged: {
            // 只在异常情况下输出警告
            if (root.width === 0 && !root_parent.isPageSwitching) {
                console.warn("⚠️ VideoPlayerStack root width became ZERO unexpectedly!")
            }
        }
        onHeightChanged: {
            // 不记录正常的高度变化
        ...
        Rectangle {
            ...
            height: 40
            // 在页面切换期间完全隐藏
            visible: !root_parent.isPageSwitching
        ...
        Rectangle {
            ...
            // 只在 root 有正确尺寸时才应用宽度变化，避免初始化时的定位错误
            width: (openSemiCircleRect.openIngState === true && root.width > 0) ? defaultWidth : 0

            Component.onCompleted: {
                var timestamp = new Date().getTime()
                var instanceId = "MenuRect_" + timestamp + "_" + Math.floor(Math.random() * 1000)
                videoPlayerMenueRectangle.objectName = instanceId
            }
            Component.onDestruction: {
                // 清理资源
            }
        ...
        onMouseHiddenChanged: {
            if (root.isActive === false)
                return;
            // 在页面切换期间不触发鼠标隐藏逻辑
            if (root_parent.isPageSwitching)
                return;
